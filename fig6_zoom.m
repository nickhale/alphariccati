% Code to generate Figure 6 of Approximate solutions to a nonlinear
...
nn = [ 1.87 1.91 1/log2(2^(1/2)+.01), 2.00001, 1/log2(2^(1/2)-.01)];

figure(1)
plot(NaN, NaN); hold on
ABC = {'H', 'I', 'J', 'K', 'L' ,'M', 'G'};
for k = 1:numel(nn)
    plot(nn(k)*[1 1], [-1, 100], '--', 'color', .5*[1 1 1], 'linewidth', 1.5);
    text(nn(k), 2.12, ['$\mathsf{' ABC{k} '}$'], 'horizontalalign', 'center', 'fontsize', 16, 'interp', 'latex');
end
plot([0 10], [0 0], 'k', 'linewidth', 2);
axis([1.85 2.1 -.1 2])
xlabel('$n$', 'interp', 'latex'); ylabel('$\|v\|_2^2$', 'interp', 'latex')
set(gca, 'fontSize', fs1), grid on

for k = 1:5
    n = nn(k);

    figure(k+1)
    % plotDecayingSoln(n); hold on

    for j = 1:2
        for direction = [1 -1]
            alp0 = 2^(1/j).*(1 + direction*1e-6);
            if ( direction  > 0 ), LS = ':'; else, LS = '-'; end
            figure(k+1)
            [vv, aalp, mvec] = followpath(alp0, n, ...
                'direction', direction, ...
                'stepinit', 1e-6, ...
                'plotting', false, ...
                'plotdata', {'color', col(j,:), 'LineStyle', LS});
            title(['$\mathsf{' ABC{k} '}: n = ' num2str(n,'%2.2f') '$'], 'interp', 'latex')
            figure(1)
            plot(1./log2(aalp), mvec, 'color', col(j,:), 'linestyle', LS);
            hold on
        end
    end

    figure(k+1)
    set(gcf, 'position', [680        1538         560         420]);
    set(gca , 'position', [0.175095235713181   0.199492061167482   0.729904764286819   0.687920637086270]);
    drawnow, pause(.1)
    print(['-f' int2str(k+1)], '-depsc', ['fig6_' int2str(k)])
...
function [vv, aalp, mvec] = followpath(alp0, nval, varargin)
...
while ( counter < maxstepno )
    ...
    if ( numel(aalp)  > 2 )
        ...
        if ( (1/log2(aalp(end)) > nval  && 1/log2(aalp(end-1)) < nval ) || ...
                ( 1/log2(aalp(end-1)) > nval  && 1/log2(aalp(end)) < nval ) )

            % Solve at precisely half-integer values of n
            v05 = newtonSolve(2.^(1./(nval)), v);
            ...
            title(['$n = ' num2str(nval,'%2.2f') '$'], 'interp', 'latex')
            set(gca, 'fontSize', 16), grid on
            ylim([0 1.5]), xlim([0 10]);

            if ( nval == 1.91 ), break, end
